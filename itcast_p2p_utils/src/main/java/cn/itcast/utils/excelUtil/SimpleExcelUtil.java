...
public class SimpleExcelUtil<T> {
	...
	public static void setDataFormatUtilInterface(DataFormatUtilInterface dataFormatUtilInterface) {
	 ...
	 * @return List<T> 你想要的？
	 */
	public List<T> getDataFromExcle(InputStream in, String sheetName, int titleCount, int colunmCount,
			MatchupData<T> callback) {
		Workbook book = null;
		List<List<Object>> tempList = new ArrayList<List<Object>>();
		try {
			book = WorkbookFactory.create(in);
		...
		for (int i = 0; i < tempList.size(); i++) {
			try {
				...
				if (s != null && s.indexOf("{") == -1) {
					throw new ExcelDataFormatException("在第 " + (i + title + 1) + " 行" + e.getMessage(), i + title + 1,
							e);
				}
		...
		for (int i = titleCount; i <= rowx; i++) {
			...
			for (int j = 0; j < colunmCount; j++) {
				...
				case XSSFCell.CELL_TYPE_BLANK: // 空白单元格
					rowlist.add(null);
					break;
				case XSSFCell.CELL_TYPE_BOOLEAN: // 布尔值单元格 For blank cells we
													// return a false
					rowlist.add(cell.getBooleanCellValue());
					isempty = false;
					break;
				case XSSFCell.CELL_TYPE_ERROR: // 错误单元格 Returns the error
												// message, such as #VALUE!
	...
	private Object getValue(int type, Cell cell) {
		...
		case Cell.CELL_TYPE_ERROR: // 错误单元格 Returns the error message, such as
									// #VALUE!
			return null; // 返回空串
	...
	/**
	 * 方法描述：处理2003版excel .xls文件 如果传入的表格名不为空 按所给的文件名字读取表格数据 如果传入 null 或 ""
		...
		for (int i = titleCount; i <= rows; i++) {
			...
			for (int j = 0; j < colunmCount; j++) {
				...
				case HSSFCell.CELL_TYPE_BLANK: // 空白单元格
					rowlist.add(null);
					break;
				case HSSFCell.CELL_TYPE_BOOLEAN: // 布尔值单元格 For blank cells we
													// return a false
					rowlist.add(cell.getBooleanCellValue());
					isempty = false;
					break;
				case HSSFCell.CELL_TYPE_ERROR: // 错误单元格 Returns the error
												// message, such as #VALUE!
					rowlist.add(null);
					isempty = false;
					break;
