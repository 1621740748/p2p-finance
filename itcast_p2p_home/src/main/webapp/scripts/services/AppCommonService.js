...
angular
		.module('AppCommonService', [ 'ngCookies', 'ui.router' ])
		.factory(
				'AuthService',
					...
					return {
						request : function(config) {
							...
							if (AuthService.getToken()) {
								config.headers.Authorization = 'xinhe '
				...
				function($http) {
					return {
						...
						checkUserName : function($str) {
							return $http.post(option.url.api_user
									+ 'validateUserName', $str, option.header);
							// 相当于$http.post('/itcast_p2p_action/user/validateUserName',
							// $str, option.header);
						},
						// 验证手机是否可用
						checkPhone : function($str) {
							return $http.post(option.url.api_user
									+ 'validatePhone', $str, option.header);
						...
						yzAuthCaptcha : function($str) {
							return $http.post(option.url.api_user
									+ 'codeValidate', $str, option.header);
							// /itcast_p2p_action/user/codeValidate
						},

						// 获取随机UUID
						getUuid : function() {
							return $http.post(option.url.api_user + 'uuid', '',
									option.header);
						...
						getImg : function($uuid) {
							return option.url.api_user
									+ "validateCode?tokenUuid=" + $uuid
						...
						getPhoneCode : function($str) {
							return $http.post(option.url.api_verification
									+ 'sendMessage', $str, option.header);
						...
						getUserSafeInfo : function($str) {
							return $http
									.post(option.url.api_user
											+ 'userSecureDetailed', $str,
											option.header);
						...
						authIdentity : function($str) {
							return $http.post(option.url.api_verification
									+ 'verifiRealName', $str, option.header);
						...
						authEmail : function($str) {
							return $http.post(option.url.api_verification
									+ 'auth', $str, option.header);
							// /itcast_p2p_action/emailAuth/auth
						},
						// 修改邮箱
						modifyEmail : function($str) {
							return $http.post(option.url.api_emailAuth
									+ 'updateEmail', $str, option.header);
							// /itcast_p2p_action/emailAuth/updateEmail
						},

						// 设置支付密码
						setPayPwd : function($str) {
							return $http.post(option.url.api_paymentpwd
									+ 'getPaymentPwd', $str, option.header);
						...
						updatePayPwd : function($str) {
							return $http.post(option.url.api_paymentpwd
									+ 'updatePaymentPwd', $str, option.header);
						...
						backPayPwd : function($str) {
							return $http.post(option.url.api_paymentpwd
									+ 'updateNewPaymentPwd', $str,
									option.header);
						},
						// 购买产品时，对帐户余额进行验证
						checkAccount : function($str) {
							return $http.post(option.url.api_investment
									+ 'checkAccount', $str, option.header);
							// /itcast_p2p_action/investment/checkAccount
						},

						// 根据产品ID获取产品的年收益率
						getYearInterest : function($str) {
							return $http.post(option.url.api_productRate
									+ 'yearInterest', $str, option.header);
						...
						modifyPwd : function($str) {
							return $http.post(option.url.api_user
									+ 'updatePassword', $str, option.header);
						},

						// 获取用户银行卡信息
						getCashInfo : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'findBankInfoByUsername', $str,
									option.header);
						},
						...
						getAllBank : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'findAllBanks', $str, option.header);
						},

						// 绑定银行卡时获取当前用户信息
						getBankUserInfo : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'findUserInfo', $str, option.header);
						},

						// 获取省份
						getProvince : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'findProvince', $str, option.header);
						},

						// 获取市区信息
						getCity : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'findCity', $str, option.header);
						},

						// 银行卡绑定
						addBankCardInfo : function($str) {
							return $http.post(option.url.api_bankcardInfo
									+ 'addBankCardInfo', $str, option.header);
						},

						// 保存借款信息
						lendSuccess : function($str) {
							return $http.post(
									'/P2P_action/userLoan/saveUserLoan', $str,
									option.header);
						},
						...
						personCenter : function($str) {
							return $http.post(option.url.api_user
									+ 'accountHomepage', $str, option.header);
						...
						statistic : function($str) {
							return $http.post(
									'/P2P_action/charges/propertyStatistics',
									$str, option.header);
						},

						// 月乘
						getMonthRide : function($str) {
							return $http.post('/P2P_action/charges/MonthRide',
									$str, option.header);
						},
						// 月取
						getMonthFetch : function($str) {
							return $http.post('/P2P_action/charges/MonthFetch',
									$str, option.header);
						},
						// 月存
						getMonthSave : function($str) {
							return $http.post('/P2P_action/charges/MonthSave',
									$str, option.header);
						},
						// 获取总的可用额
						getTotalRevenue : function($str) {
							return $http.post(
									'/P2P_action/charges/totalRevenue', $str,
									option.header);
						},

						// 购买中的计划
						getProductAccountBuying : function($str) {
							return $http.post(option.url.api_charges
									+ 'ProductAccountBuying', $str,
									option.header);
						},
						...
						record : function($str) {
							return $http
									.post(
											'/P2P_action/accountrecord/findMoreAccountRecord',
											$str, option.header);
						},

						getAbleAccout : function() {
							return $http.post(option.url.api_extracts
									+ 'extractMoney', '', option.header);
						...
						withdraw : function($str) {
							return $http.post(option.url.api_charges
									+ 'addMayTake', $str, option.header);
							// /itcast_p2p_action/charges/addMayTake
						},

						// 获取当前用户所有的站内信息
						getAllMsg : function($str) {
							return $http.post('/itcast_p2p_action/api/list', $str,
									option.header);
						},

						// 退出
						logout : function($str) {
							return $http.post(option.url.api_user + 'logout',
									$str, option.header);
						...
						getPhoneCodeByToken : function() {
							return $http.post(option.url.api_verification
									+ 'tokensendSMS', '', option.header);
						},

						// 手机绑定
						addPhoneAuth : function($str) {
							return $http.post(option.url.api_verification
									+ 'addPhone', $str, option.header);
						},
						// 修改手机后绑定
						updatePhone : function($str) {
							return $http.post(option.url.api_user + 'addPhone',
									$str, option.header);
						},
		...
		.factory(
				...
				function($http, AuthService) {
					return {
						// 登录
						signIn : function($str) {
							return $http.post(option.url.api_user + 'login',
									$str, option.header);
						...
						register : function($str) {
							return $http.post(option.url.api_user + 'signup',
									$str, option.header);
		...
		.factory(
				...
				function() {
					...
					function checkMoney(money) {
						if (!money.replace(/\s/g, '') || money == '0.00'
								|| money == 0 || money == '0.0') {
							return {
								'msg' : '金额数不可为空或0',
								'isTrue' : false
							};
						} else if (!/^([1-9][0-9]*(\.[0-9]{1,2})?|0\.[0-9]{1,2})$/
								.test(money)) {
							return {
				...
				[
						...
						function($state, AuthService) {
							...
							Controller_Class.fn = Controller_Class.prototype = {
								...
								version : '1.0',
								getListData : function(func, PostService, parm,
										scope) {
									PostService[func]
											(parm)
											.success(
								...
								/*
								 * @description 判断ie版本 @params isIE6 isIE7 isIE8
								 */
								isIEVersion : function(status) {
									var isIE = !!window.ActiveXObject, _version = {
										isIE6 : function() {
											return isIE
								...
								cookie : {
									setCookie : function(name, value, time) {
										var strsec = time ? this.getsec(time)
												: 0, exp = new Date();
								...
								/*
								 * @description 给对象数组或数字数组进行排序，对象数组中有一项是数字 @use
								 * [].sort(hmd.sort);
								 */
								sort : function(a, b) {
									if (typeof a === 'object') {
								...
								/*
								 * @description 将一个串转化成数组 @param str eg:
								 * 'pProductName=11&pAmount=22' @param
								 * separator1 第一个分隔符，比如像上面的'&',如果不传此参数，默认为'&'
								 * @param separator2
								 * 第二个分隔符，比如像上面的'=',如果不传此参数，默认为'=' @return
								 * 会返回这样格式的数组:[{'name' : 'pProductName','value' :
								...
								/*
								 * @description 错误信息显示 @param code 就是后台返回的
								 * status @param status
								 * 三个状态error(错误图标),ok(成功图标),attention(警告图标)
								 */
								popupErrorInfo : function(code, status) {
									var _doc = document, _div = _doc
											.createElement('div'), _width = $(
											_doc.body).width() / 2, _height = $(
											_doc.body).height() / 2, current_class;
								 ...
								 */
								managePostService : function(func, PostService,
										parm, scope, callback) {
									var self = this;
									PostService[func]
											(parm)
								...
								/** @description 导出统一方法 */
								exportPostService : function(func, PostService,
										parm, scope) {
									PostService[func](parm);
								},

								/** @description 导出的参数要根据查询按钮的属性来设置 */
								setExportParamsByQueryAttr : function($obj,
										param) {
									for ( var i in param) {
										$obj.attr(i, param[i]);
									}
							...
							$
									.extend(
											Controller_Class.fn,
											{
												...
												getPieAllDataByProduct : function(
														_arr) {
													$('#donutchart')
															.highcharts(
																	{
												 ...
												 * @description 画折线
												 */
												getLineAllDataByProduct : function(
														_s) {
													var _str = !!_s ? _s
															: '月取计划'
													var oid = 'barchart';
													...
													var data = !!this.barchart ? this.barchart
															: [];
													if (data.length > 0) {
														var data_sort = [], data_time = [], data_time_new = [], data_time_copy = [], min_value, max_value, data_money = [];
														$(data)
																.each(
																		function(
																				index,
																				element) {
																			var UExpectedDate = element.UExpectedDate, _arr = UExpectedDate
																					.split('-'), _year = _arr[0], _m = _arr[1], _month = _m.length == 1 ? '0'
																					+ _m
																					: _m, _date = _year
																					+ '.'
																					+ _month;
																			data_sort
																					.push({
																						index : _date,
																						value : !!element.UExpectedMoney ? element.UExpectedMoney
																								: 0
																					})
